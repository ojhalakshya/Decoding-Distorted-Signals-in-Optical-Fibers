...
query = input('Enter Encoding (NRZ) or PAM')
path_train = 'data/data_Binary_NRZ_RX(small).csv'
path_label = 'data/labels_Binary_NRZ_TX.csv'
model_dir = "/tmp/dilated_cnn_model_binary"

if query == 'PAM':
    path_train = 'data/data_PAM4_RX(small).csv'
    path_label = 'data/labels_PAM4_TX.csv'
    model_dir = "/tmp/dilated_cnn_model_pam4"

# parse the csv files
data = dilated_neural_net.csv_parser(path_train, 1, 7, np.float32)
labels = dilated_neural_net.csv_parser(path_label, 0, 0, np.int32)
